...
function renderWeekApiLinks() {
  ...
  container.innerHTML = sourceLink("arome", "https://portail-api.meteofrance.fr/web/fr/api/ARPEGE", "ARPEGE")
    ...
    + sourceLink("openMeteo", "https://open-meteo.com/en/docs", "ECMWF / Open-Meteo");
  refreshSourceIndicators();
}
...
function weekModelAgreement(ecmwf, arpege) {
  ...
  const storm = item => Number(item.weatherCode) >= 95;
  const rainProfiles = [
...
function weekForecastEvolution() {
  const fields = day => ({
    ...
    weatherCode: Number(day.weatherCode), rainPeriods: Array.isArray(day.rainPeriods) ? day.rainPeriods.slice().sort() : null,
  ...
  const changesBetween = (beforeModels, afterModels, date) => {
    ...
    ["ecmwf", "arpege"].forEach(model => {
      ...
      if ((after.weatherCode >= 95) !== (before.weatherCode >= 95)) changes.add("orage");
    });
  ...
  dates.forEach(date => {
    ...
    const description = level === "frequent" ? "Changements fréquents sur : " + [...changes].join(", ") + "."
      ...
      : "";
    byDate.set(date, { level, description, changeRate, changedCount: changedTransitions.length, transitionCount: transitions.length });
  });
...
function renderWeekForecast() {
  ...
  const weekDayHeading = (date, dateKey, ...days) => {
    ...
    return '<div class="week-day-head week-day-head-active"><strong>Aujourd’hui,</strong><time class="week-active-start" datetime="' + escapeText(start) + '">à partir de ' + escapeText(hour) + '</time></div>';
  };
  ...
  const renderDay = (day, sourceKey, sourceControls) => {
    ...
    const iconRain = code >= 95 ? Math.max(.5, dailyRainIconAmount(dailyRain)) : displayRain ? dailyRainIconAmount(dailyRain) : 0;
  ...
  const renderSynthesisDay = (ecmwf, arpege, sourceControls) => {
    ...
    const stormModels = [Number(ecmwf.weatherCode) >= 95 ? "Open-Meteo" : "", Number(arpege.weatherCode) >= 95 ? "Météo-France" : ""].filter(Boolean);
    ...
    const evolutionStabilityScore = Number.isFinite(evolution.changeRate) ? 1 - evolution.changeRate : null;
    const evolutionMarkup = '<span class="week-footer-status ' + evolutionTone + '" title="' + escapeText(evolutionTitle) + '"><span>Évolution des prévisions</span><span role="img" aria-label="' + escapeText(evolutionLabel + " : " + evolutionTitle) + '">' + statusDots(evolutionStabilityScore) + '</span></span>';
...
function renderActiveForecast() {
  ...
  if (!data) return;
  const meteoFrance = withHourlyNowcast(data.arome, piafHourlyRain(data.piaf));
...
function renderThreatMap(radar, lightning = null, mapRadiusKm = activeNowcastMapRadius) {
  ...
  const cells = radarCells.map((cell, index) => {
    ...
    return '<g><circle class="radar-cell chart-point' + (selected ? ' selected' : '') + '" cx="' + x(cell.eastKm).toFixed(1) + '" cy="' + y(cell.northKm).toFixed(1) + '" r="' + radius.toFixed(1) + '" tabindex="0" aria-label="' + escapeText(name) + '" data-tooltip="' + escapeText(detail) + '"></circle><text class="cell-name' + (selected ? '' : ' secondary') + '" x="' + (x(cell.eastKm) - radius - 4).toFixed(1) + '" y="' + (y(cell.northKm) - radius - 4).toFixed(1) + '" text-anchor="end">' + escapeText(cellId) + '</text></g>';
  }).join('');
...
function renderRadarNowcast(radar, piaf, arome, lightning) {
  ...
  const piafRainTrend = values => {
    const series = values.map(item => Math.max(0, Number(item.precipitation) || 0));
    ...
    if (!wetIndexes.length || total <= 0) return { label: "stable", change: 0, detail: "Aucune pluie prévue par PIAF · " + stepDetail };
    ...
    if (projectedChange < -threshold) {
      ...
      return { label: "decroissant", change: projectedChange, detail: wording + " selon PIAF · " + timing };
    }
    const wording = firstWet > 0 && lastWet < series.length - 1 ? "Passage pluvieux temporaire" : "Pluie globalement stable";
    return { label: "stable", change: projectedChange, detail: wording + " selon PIAF · " + timing };
  };
